...
将所有硬编码的"全部使用中文"替换为语言自适应指令。
这是一个纯 prompt 改动，基于 phase5_day9_skeleton.py 的最小 diff 版本。

注意：本文件只展示需要改动的提示词部分，完整运行需要配合其他优化文件的状态定义和节点代码。
"""
...
# ===== P3 改动 3：section_writer 使用语言自适应 =====
...
# ===== P3 改动 5：intro_conclusion 使用语言自适应 =====
...
def write_section(state: InterviewState):
    context = state["context"]
    analyst = state["analyst"]
    # P3 改动 6：传入 language_instruction
...
if __name__ == "__main__":
    # 测试：用英文主题
    topic = "AI applications in healthcare: benefits and challenges"
    max_analysts = 2

    print(f"=== P3 优化测试（语言自适应）：{topic} ===\n")
    config = {"configurable": {"thread_id": "test_p3"}}

    result = graph.invoke({"topic": topic, "max_analysts": max_analysts}, config)
    print(f"生成了 {len(result['analysts'])} 个分析师")
    ...
    for event in graph.stream(None, config, stream_mode="updates"):
        for node_name, node_output in event.items():
            if node_name == "finalize_report":
                ...
                # 检查报告语言
                has_chinese = any('一' <= c <= '鿿' for c in report[:500])
                print(f"  最终报告 ({len(report)} 字符)")
                print(f"  报告前500字包含中文: {has_chinese}")
                print(f"\n{'='*60}")
                print("报告开头：")
                print(report[:300])
                print(f"{'='*60}")
