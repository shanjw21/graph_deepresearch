...
在 search_web 和 search_baike 节点中对长内容做 LLM 摘要。
基于 lesson_notes/phase5_day9_skeleton.py 的改动版本。

注意：本文件依赖 P1 的去重逻辑，是 P1 + P2 的组合版本。
"""
...
def summarize_content(content: str, max_length: int = 300) -> str:
    ...
    system_prompt = summarize_instructions.format(content=truncated)
...
def search_web(state: InterviewState):
    ...
    try:
        ...
        if summarized_docs:
            ...
            formatted_docs = f"<Document />搜索结果的URL均已见过，查询: {search_query.search_query}</Document>"
...
def search_baike(state: InterviewState):
    ...
    try:
        ...
        if summarized_docs:
            ...
            formatted_docs = f"<Document />百科搜索结果的URL均已见过，查询: {search_query.search_query}</Document>"
...
print("✅ 访谈子图编译完成（含 URL 去重 + 摘要 + reflect 节点）")
...
if __name__ == "__main__":
    ...
    print(f"=== P2 优化测试（摘要）：{topic} ===\n")
    config = {"configurable": {"thread_id": "test_p2"}}
    ...
    for event in graph.stream(None, config, stream_mode="updates"):
        for node_name, node_output in event.items():
            if node_name == "conduct_interview":
                ...
                print(f"  [摘要] context 总长度: {total_chars} 字符（摘要后）")
