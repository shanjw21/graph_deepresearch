"""
阶段五 · Day 9 任务：Map-Reduce（下）—— Reduce 三路并行 + finalize_report

在 Day 8 的主图骨架基础上，实现 4 个 Reduce 节点：
1. write_report（sections → 报告主体）
2. write_introduction（sections → 引言）
3. write_conclusion（sections → 结论）
4. finalize_report（引言 + 主体 + 结论 → 完整报告）

然后端到端运行完整系统。

参考源码：research_assistant.py 第 468-536 行、894-1043 行
"""
...
# ============================================
# 提示词模板（Day 6-8 的 + 新增 2 套 Reduce 提示词）
# ============================================
...
def write_report(state: ResearchGraphState):
    ...
    提示：
    1. 从 state 获取 sections 和 topic
    2. sections 是 list，用 "\n\n".join() 合并成字符串
    3. 用 report_writer_instructions.format(topic=..., context=...) 格式化
    4. 调用 llm.invoke([SystemMessage(...), HumanMessage(content="基于这些备忘录撰写一份报告。")])
    5. 返回 {"content": report.content}
...
def write_introduction(state: ResearchGraphState):
    ...
    1. 从 state 获取 sections 和 topic
    2. sections 合并成字符串
    3. 用 intro_conclusion_instructions.format(topic=..., formatted_str_sections=...) 格式化
    4. 调用 llm.invoke([SystemMessage(...), HumanMessage(content="撰写报告引言")])
    5. 返回 {"introduction": intro.content}
...
def write_conclusion(state: ResearchGraphState):
    ...
    2. sections 合并成字符串
    3. 用 intro_conclusion_instructions.format(topic=..., formatted_str_sections=...) 格式化
    4. 调用 llm.invoke([SystemMessage(...), HumanMessage(content="撰写报告结论")])
    5. 返回 {"conclusion": conclusion.content}
...
def finalize_report(state: ResearchGraphState):
    """
    TODO: 组装最终报告（不调用 LLM，纯字符串拼装）

    提示：
    1. 从 state 获取 content
    2. 如果 content 以 "## Insights" 开头，去掉它
    3. 如果 content 包含 "## Sources"，用 split 分离出 sources
    4. 拼装：introduction + "\n\n---\n\n" + content + "\n\n---\n\n" + conclusion
    5. 如果 sources 存在，追加 "\n\n## Sources\n" + sources
    6. 返回 {"final_report": final_report}
...
if __name__ == "__main__":
    topic = "AI在医疗领域的应用与挑战"
    max_analysts = 2  # 先用 2 个分析师，省 API 费用

    print(f"=== 端到端测试：{topic} ===\n")

    config = {"configurable": {"thread_id": "test_day9"}}

    # 第1步：启动 → 在 human_feedback 前暂停
    print("--- 第1步：生成分析师 ---")
    result = graph.invoke({
        "topic": topic,
        "max_analysts": max_analysts
